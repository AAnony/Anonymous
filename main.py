...
def fl_finetune(
        ...
        local_learning_rate: float = 1e-4,#3e-4
...
#        global_model,
    ...
    def tokenize(prompt, add_eos_token=True):
        '''
        The returned value of the result is a dictionary, which includes:
        - input_ids: The encoded representation of the tokenized text, represented as a list of integers.
        - attention_mask: A binary mask indicating which tokens are actual input tokens and which ones are padding tokens. It has the same length as input_ids.
        - token_type_ids: An identifier used to distinguish between different sequences of text, such as sentence A and sentence B. In this case, since there is only one input sequence, this key is not used.
        - Other keys: Depending on the specific tokenizer type and parameter settings, there may be additional keys included.
        '''
        result = tokenizer(
            prompt,
            truncation=True,
    ...
    if central == True:
        print("start train central model")
        eepoch = int(client_selection_frac*local_num_epochs*num_communication_rounds)
        for epoch in range(eepoch):
            client = GeneralClient(999, model, data_path, output_dir)
            ...
            print(os.path.join(output_dir, "C/adapter_model_{}.bin".format(str(epoch))))
            torch.save(model.state_dict(), os.path.join(output_dir, "C/adapter_model_{}.bin".format(str(epoch))))
